...
** DESCRIPTION
	this program reads 1 streamflow minute data and pulls out peaks with
 	...
 	a number of peaks per year over the specified minimum thresholds
	input files: 	sf1min the 1 minute streamflow data file, columns are: stnid year month day daysince1960 hour minute streamflowcfsvalue
					temparea2 area of the watershed
					len length of the streamflow record in years
	output files: 	POT information for peaks over threshold values with date/time of the max value, columns are: year month day daysince1960 hour minute streamflowcfsvalue
					YNPOTpy "yes" or "no" for if the stream data is worth saving, as determined by the min_ppy (minimum peaks over threshold per year) 
**************************************************/
...
/***  set threshold values ***/
	...
	min_ppy = 1; /* minimum number of peaks per year to save basin data */
/**********************************************************************/

/* set and open 1 minute list file */
 	if((infile=fopen("sf1min","r")) == NULL)  {
    		printf("cannot open peaks infile \n");
      		exit(1);    }

/* set and open area file */
 	if((areafile=fopen("temparea2","r")) == NULL)  {
    		printf("cannot open peaks areafile \n");
      		exit(1);    }

/* set and open length of record in years file */
 	if((lengthfile=fopen("len","r")) == NULL)  {
    		printf("cannot open  file \n");
      		exit(1);    }

/* open peaks output file */
  	if((outfile=fopen("POT","w")) == NULL)  {
    		printf("cannot open output file \n");
      		exit(1);    }

/* open flashy output file */
  	if((flashyfile=fopen("YNPOTpy","w")) == NULL)  {
    		printf("cannot open output file \n");
      		exit(1);    }
