...
** DESCRIPTION
	...
	input files: 	interp_in streamflow file for one gage, columns are: stnid year month day hour minute cfsstreamflowvalue
	output files: 	sf1min 1 minute interpolated streamflow, columns are: stnid year month day dayfrom1960 hour minute cfsstreamflowvalue
					len length of data record in years with missing data (over 90 days) subtracted 
**************************************************/
...
{  
    FILE  *lengthfile,*infile,*outfile;
    char filename[20],input[200],stnid[20],tz[5],junk1[300];
    int i,j,k;
    float jtime0,day0,hour0,minute0,value0,jday0,miss_yr;
    float jtime,day,hour,minute,value,jday,day1,jtimea;
    int year,year0,month,month0;
    float tint, vint;
    int ind1,ind2,ind3;
    float pday,pminute,len_yr;
    int firstday[13],dayyear[55][13],nyears,phour;
    int firstdayofyear[55],pyear,pdoy,pmonth,mdoy;
	...
	len_yr=1.;
  /*** open output file ***/
	if((outfile=fopen("sf1min","w")) == NULL)  {
		printf("cannot open output file \n");
		exit(1);    
	}

 /***** open input file ******/
	if((infile=fopen("interp_in","r")) == NULL)  {
...
/*** first day of year for each of years ***/
...
/*** read first line of input file and create values for the first line***/
	...
	for(k=0;k<16;k++) { 	/* check for leap year */
		if(year0==leapyear[k]) 
			jday0 = firstdayofyear[year0-1960] + dayleap[month0-1]-1 + day0; 
	}
	jtime0 = (jday0 - day1)*1440. + hour0*60. + minute0; /* first day of dataset in minutes from 1960 */
	jtimea=jtime0;
	 	 
	fprintf(outfile,"%s %d %4d %4.0f %4.0f %3.0f %3.0f %f \n",stnid,year0,month0,day0,day0,hour0,minute0,value0);
...
/*** increment through the file and interpolate for the intervals ***/ 
	while (!feof(infile)) {
		...
		for(k=0;k<16;k++) { /* check for leap year */
			if(year==leapyear[k]) 
				jday = firstdayofyear[year-1960] + dayleap[month-1]-1 + day; 
		}
		...
		for (i=0;i<tint;i++){ /* interpolate interval down to 1 minute */
			...
			fprintf(outfile,"%s %d %4d %4d %4.0f %d %3.0f %f \n",stnid,pyear,pmonth,mdoy,jtime/1440+1,phour,pminute,value);
		}  /*  closes 1 minute for loop */
		...
		/* Every month calculate length of time for which there is data (excluding when there are over 90 day gaps) and print it to length file */
			if (mdoy==1){
				len_yr = (jtime - jtimea)/1440./365. - miss_yr; 
				if((lengthfile=fopen("len","w")) == NULL)  {
					printf("cannot open output file \n");
					exit(1);    
				}
				fprintf(lengthfile,"%f \n",len_yr);
				fclose(lengthfile);
			} /*closes lengthfile loop */
